...
subscription_periods AS (
    SELECT
        ...
        CASE
            WHEN start_month = end_month THEN DATEADD('month', 1, {{ truncate_monthly('ends_at') }})
...
mrr_with_changes AS (
    SELECT
        *,
        ...
        {{ lagged_instance('is_subscribed_current_month', 'date_month', 'user_id, subscription_id', default_value='FALSE') }} AS is_subscribed_previous_month,

        {{ lagged_instance('mrr', 'date_month', 'user_id, subscription_id', default_value='0.0') }} AS previous_month_mrr_amount,
